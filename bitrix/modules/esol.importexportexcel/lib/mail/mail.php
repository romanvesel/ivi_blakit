			...
			while(!empty($arFolders) && !$fid)
			{
				...
				if(!empty($mailsIds))
				{
					...
					while($i>=0 && !$break)
					{
						...
						elseif($this->params['FILENAME_REGEXP']=='Y' && strpos($this->params['FILENAME'], '//')!==false && ($arMailData = $this->imap->getMessage($folder, $mailId, 'DATA', ($error='')))!==false)
						{
							if(preg_match('/'.str_replace('/', '\/', $arCriterias['FILENAME']).'/i', $arMailData['TEXT'], $m))
							{
								if(!$this->params['LAST_DATE'] || $arMailData['DATE']!=$this->params['LAST_DATE'])
								{
									$arFile = \CKDAImportUtils::MakeFileArray($m[0], $maxTime);
