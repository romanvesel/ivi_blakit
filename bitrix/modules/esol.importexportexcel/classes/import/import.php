...
class CKDAImportExcel {
	...
	public function AddElement($arFieldsElement)
	{
		...
		if($ID)
		{
			if(isset($arFieldsElement['ID']) && isset($arFieldsElement['TMP_ID']))
			{
				...
				if(\Bitrix\KdaImportexcel\DataManager\IblockElementIdTable::update($arFieldsElement['TMP_ID'], array('ID'=>$arFieldsElement['ID'])))
				{
	...
	public function SaveSKUWithGenerate($ID, $NAME, $IBLOCK_ID, $arItem)
	{
		...
		if(!empty($this->fieldsForSkuGen))
		{
			...
			foreach($filedList as $key=>$field)
			{
				...
				if((preg_match('/^OFFER_(IE_PREVIEW_PICTURE|IE_DETAIL_PICTURE|ICAT_QUANTITY|ICAT_PURCHASING_PRICE|ICAT_PRICE\d+_PRICE|ICAT_STORE\d+_AMOUNT|ICAT_WEIGHT)$/', $field) || in_array($key, $this->fieldsBindToGenSku) || in_array($field, $arGenFields)) && strpos(preg_replace('/\{[^\}]*\}/', '', $val), $this->params['ELEMENT_MULTIPLE_SEPARATOR'])!==false)
				{
					$arItem['~~'.$key] = $valOrig;
					$arItem[$key] = $val;	
	...
	public function GetStoreAmountList(&$arFieldsProductStores, $arParams, $value)
	{
		...
		foreach($arStores as $strStore)
		{
			...
			{
				$dbRes = \Bitrix\Catalog\StoreTable::getList(array('filter'=>array('LOGIC'=>'OR', array('TITLE'=>$storeName), array('ADDRESS'=>$storeName), array('CODE'=>$storeName)), 'select'=>array('ID')));
