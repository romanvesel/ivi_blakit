...
var EIXPreview = {
	Init: function()
	{
		...
		if(params.FIELDS)
		{
			...
			var selectNames = ['section_fields', 'subsection_fields', 'element_fields', 'property_fields', 'offproperty_fields', 'ibproperty_fields'];
	...
	ShowBaseElements: function(link)
	{
		...
		if(parentBE)
		{
			...
			if(parentBE=='ELEMENT') arElems = ['PROPERTY', 'OFFER', 'OFFPROPERTY', 'SECTION'];
	...
	ShowBaseElemSettings: function(e, btn)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+esolIXModuleFilePrefix+'_group_'+fieldsCode.toLowerCase()+'.php?PROFILE_ID='+form.PROFILE_ID.value,
			'width':'900',
			'height':'400',
			'resizable':true,
			'content_post':{'GROUP': fieldsCode, 'XPATH': xpath, 'FIELDS': fields, 'INNER_GROUPS': innerGroups, 'MAP': map}
		};
	...
	ShowFieldSettings: function(e, btn)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+esolIXModuleFilePrefix+'_field_settings.php?field='+val+'&field_name='+name+'&xpath='+xpath+'&index='+index+'&PROFILE_ID='+form.PROFILE_ID.value,
			'width':'900',
			'height':'400',
			'resizable':true,
	...
	{
		$('#esol_ix_xml_wrap input[name="SETTINGS['+group+'_MAP]"]').val(data);
		if(BX.WindowManager.Get()) BX.WindowManager.Get().Close();
	}
}
...
var EProfile = {
	...
	ShowMassUploader: function()
	{
		var dialog = new BX.CAdminDialog({
			...
			'content_url':'/bitrix/admin/'+esolIXModuleFilePrefix+'_mass_uploader.php',
			'width':'900',
			'height':'450',
			'resizable':true});
...
var ESettings = {
	...
	ShowValuesFromFile: function(link, prefileId, xpath)
	{
		var wait = BX.showWait();
		$.post(window.location.href, {'MODE': 'AJAX', 'ACTION': 'GET_XPATH_VALUES', 'XPATH': xpath, 'PROFILE_ID': prefileId}, function(data){
			eval('var res = '+data+';');
			if(typeof res == 'object')
			{
				...
				$(link).closest('td').html('<textarea readonly>'+vals+'</textarea>');
			}
			BX.closeWait(null, wait);
		});
	},
	
	ShowSelectMapping: function(link)
	{
