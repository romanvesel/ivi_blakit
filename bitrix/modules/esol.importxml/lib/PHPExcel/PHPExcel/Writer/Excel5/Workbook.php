...
class KDAPHPExcel_Writer_Excel5_Workbook extends KDAPHPExcel_Writer_Excel5_BIFFwriter
{
	...
	function _calcSheetOffsets()
	{
		...
		foreach ($this->_phpExcel->getWorksheetIterator() as $sheet) {
			$offset += $boundsheet_length + strlen(KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
		}
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$name = substr(KDAPHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);
