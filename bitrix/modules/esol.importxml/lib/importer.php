...
class Importer extends ImporterBase {	
	function __construct($filename, $params, $fparams, $stepparams, $pid = false)
	{
		...
		if(isset($this->sectionMap['SECTION_LOAD_MODE']) && strlen($this->sectionMap['SECTION_LOAD_MODE']) > 0)
		{
			...
			else
			{
				if($this->params['GROUPS']['SECTION'])
				{
					...
					while($arItem = $this->GetNextSectionRecord($time))
					{
						...
						{
							if(($this->elementInSection && $this->stepparams['xmlCurrentRowInSection'] > 0) || ($this->subSectionInSection && $this->stepparams['xmlSubsectionCurrentRowInSection'] > 0))
							{
	...
	public function EndOfLoading($time)
	{
		...
		{
			$arOfferDefaults = unserialize(base64_decode($this->params['OFFER_MISSING_DEFAULTS']));
			if(!is_array($arOfferDefaults)) $arOfferDefaults = array();
		}
		...
		$oProfile = \Bitrix\EsolImportxml\Profile::getInstance();
		$arEventData = $oProfile->OnEndImport($this->GetFileName(), $this->stepparams, $this->errors);
	...
	public function MissingElementsUpdate($ID, $IBLOCK_ID, $isOffer = false)
	{
		...
		if(!empty($arDefaults))
		{
			...
			$this->AfterSaveProduct($arElemVals, $ID, $IBLOCK_ID, true);
			if(!empty($arElemVals))
			{
	...
	public function GetNextSectionRecord($time=0)
	{
		...
		while(isset($this->xmlSections[$this->xmlSectionCurrentRow]) && ($moveCnt < count($this->xmlSections)))
		{
			...
			{
				if(array_key_exists('ISECT_PARENT_TMP_ID', $arItemFields) && $arItemFields['ISECT_PARENT_TMP_ID'] && !isset($this->sectionIds[$arItemFields['ISECT_PARENT_TMP_ID']]) && !isset($this->sectionMap['MAP'][$arItemFieldsOrig['ISECT_TMP_ID']]))
				{
					$break = true;
				}
				if(array_key_exists('ISECT_TMP_ID', $arItemFields) && $arItemFields['ISECT_TMP_ID'] && isset($this->sectionIds[$arItemFields['ISECT_TMP_ID']]) && !$this->subSectionInSection)
				{
					$unset = true;
					$break = true;
				}
	...
	public function SaveSectionRecord($arItem, $parentSectionId=0)
	{
		...
		foreach($this->params['FIELDS'] as $key=>$fieldFull)
		{
			...
			$prefix = (((is_numeric($parentSectionId) && $parentSectionId > 0) || (!is_numeric($parentSectionId) && strlen($parentSectionId) > 0)) ? 'ISUBSECT_' : 'ISECT_');
		...
		if($this->sectionLoadMode || count($this->sectionMap['MAP']) > 0)
		{
			...
			{
				$sectionID = (int)current($this->sectionMap['MAP'][$arItemFields['TMP_ID']]);
			}
			if($sectionID > 0)
			{
				$this->sectionIds[$arItemFields['TMP_ID']] = $sectionID;
				$this->SaveSectionRecordAfter($sectionID, $arItem);
				$this->SaveStatusImport();
				return $sectionID;
			}
			if($this->sectionLoadMode=='MAPPED') return false;
			if($this->sectionLoadMode=='MAPPED_CHILD' && !array_key_exists($arItemFields['PARENT_TMP_ID'], $this->sectionIds)) return false;
		}
		...
		if($this->useSectionPathByLink)
		{
			...
			$this->SaveSectionRecordAfter($arFieldsSections['TMP_ID'], $arItem);
			$this->SaveStatusImport();
	...
	public function SaveRecord($arItem, $sectionID=0)
	{
		...
		$this->AddGroupsProperties($arFieldsElement, $arFieldsElementOrig, $arFieldsProps, $arFieldsPropsOrig, $arFieldsPrices, $arFieldsProductStores, $arFieldsProductDiscount, $arFieldsProduct, $IBLOCK_ID);
		...
		if($this->params['ELEMENT_UID_SKU']) 
		{
			...
			{				
				...
				while($arOfferItem = $this->GetNextOffer($ID, $arItem))
				{
					...
					$this->SaveSKUWithGenerate($ID, $arFieldsElement['NAME'], $IBLOCK_ID, $arOfferItem, $arFParams);
				}
				$this->currentXmlObj = $this->currentParentXmlObj;
			...
			if(!$isSaved)
			{
				$this->SaveSKUWithGenerate($ID, $arFieldsElement['NAME'], $IBLOCK_ID, $arItem, $arFParams);
			}
		}
	...
	public function AddGroupsProperties(&$_e, &$_oe, &$_p, &$_op, &$_c, &$_s, &$_d, &$_pr, $IBLOCK_ID, $isOffer=false)
	{
		...
		while($arProperty = $this->GetNextProperty($groupXpath, $groupName))
		{
			...
			foreach($this->params['FIELDS'] as $key=>$fieldFull)
			{
				...
				$fieldName = substr($field, strlen($groupName) + 1);
				if($fieldName=='TMP_ID') $tmpID = $value;
				else $arPropertyFields[$fieldName] = $value;
				$arPropertyFieldsOrig[$fieldName] = $this->Trim($valueOrig);
			}
			...
			if(!$isOffer)
			{
				if($this->propertyMap['NOT_LOAD_WO_MAPPED']=='Y') $propIds = array();
				if($this->propertyMap['PROPERTY_NOT_CREATE']=='Y') $allowCreate = false;
				if($this->isPropertyMap && array_key_exists('NAME', $arPropertyFieldsOrig))
				{
					if(array_key_exists($arPropertyFieldsOrig['NAME'], $this->propertyMap['MAP']))
					{
						$propIds = array();
						$arFields = $this->propertyMap['MAP'][$arPropertyFieldsOrig['NAME']];
						foreach($arFields as $field)
						{
							$value = $origValue = $arPropertyFields['VALUE'];
							if(strpos($field, 'OFFER_')===0)
							{
								$this->AddGroupsField($__e, $__oe, $__p, $__op, $__c, $__s, $__d, $__pr, substr($field, 6), $value, $origValue, true);
							}
							else
							{
								$this->AddGroupsField($_e, $_oe, $_p, $_op, $_c, $_s, $_d, $_pr, $field, $value, $origValue);
								if(strpos($field, 'IP_PROP')===0)
								{
									$propIds[] = substr($field, 7);
								}
							}
						}
					}
			...
			foreach($propIds as $propId)
			{
				...
				elseif($arPropertyFields['NAME']) $arProp = $this->GetIblockPropertyByName($arPropertyFields['NAME'], $IBLOCK_ID, $allowCreate);
				...
				if(is_array($arProp) && isset($arProp['ID']))
				{
					...
					$this->GetPropField($_p, $_op, $this->fparams[$key], $currentPropDef, $fieldName, $value, $origValue, $this->params['ELEMENT_UID']);
	...
	public function AddGroupsField(&$_e, &$_oe, &$_p, &$_op, &$_c, &$_s, &$_d, &$_pr, $field, $value, $origValue, $isOffer=false)
	{
		if(strpos($field, 'IE_')===0)
		{
			$fieldKey = substr($field, 3);
			...
			if(isset($_e[$fieldKey]) && in_array($field, ($isOffer ? $this->params['ELEMENT_UID_SKU'] : $this->params['ELEMENT_UID'])))
			{
				if(!is_array($_e[$fieldKey]))
				{
					$_e[$fieldKey] = array($_e[$fieldKey]);
					$_oe[$fieldKey] = array($_oe[$_oe]);
				}
				$_e[$fieldKey][] = $value;
		...
		elseif(strpos($field, 'IP_PROP')===0 && $isOffer)
		{
			$fieldName = substr($field, 7);
			if($arOfferIblock = $this->GetCachedOfferIblock($this->params['IBLOCK_ID']))
			{
				...
				$this->GetPropField($_p, $_op, array(), $propsDef[$fieldName], $fieldName, $value, $origValue);
			}
		}
	}
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array(), $arFParams=false)
	{
		...
		$this->AddGroupsProperties($arFieldsElement, $arFieldsElementOrig, $arFieldsProps, $arFieldsPropsOrig, $arFieldsPrices, $arFieldsProductStores, $arFieldsProductDiscount, $arFieldsProduct, $OFFERS_IBLOCK_ID, true);
		...
		if($dbRes->SelectedRowsCount()==0 && $ID && $this->params['SEARCH_OFFERS_WO_PRODUCTS']!='Y')
		{
			if($this->params['ONLY_UPDATE_MODE_ELEMENT']!='Y')
			{
				...
				if($OFFER_ID = $this->AddElement(array_merge($arFieldsElement, array('PROPERTY_VALUES'=>array($OFFERS_PROPERTY_ID => $ID)))))
				{
	...
	public function SaveProperties($ID, $IBLOCK_ID, $arProps, $needUpdate = false)
	{
		...
		foreach($arProps as $k=>$prop)
		{
			if(!is_array($prop))
			{
				$this->PropReplaceId($arProps[$k]);
			}
			else
			{
				array_walk_recursive($arProps[$k], array($this, 'PropReplaceId'));
			}
		}
